...
class DRQN(nn.Module):
    def __init__(self, n_predicates, n_entities, n_responses, num_actions, gru_size=32, bidirectional=False):
        ...
        # self.gru = nn.GRU(self.o_embed.observation_size + n_entities - 1, self.gru_size, num_layers=1, batch_first=True,
...
class AgentDRQNSA(Agent):
    ...
    def __init__(self, dataset: InternalKB, switch_thres, config, n_chances=20, lr=5e-4, load_path=None, mode='train'):
        self.n_responses = 3 
        super(AgentDRQNSA, self).__init__(dataset, n_chances, switch_thres)
    ...
    def reset(self):
        ...
        self.episode_log = {'question':[], 'question_flat':list(), 'response':[]}  # intialize a first observation
    ...
    def update_response(self, response):
        ...
        _, _ = self.guess_generate_incremental(self)
    ...
    def question(self):
        ...
        if done and self.guess_right:
            # update the multi-nouli parameters
            # self.KB_update(self.episode_log['question'], self.episode_log['response'], True)
            self.KB_update_periodic(self.guess, self.episode_log['question'], self.episode_log['response'], 1)
    ...
    def env_step(self):
        ...
        r += reward_func_intrinsic(self.posterior, self.posterior_last, ratio=10) if self.config.intr_reward == True else 0.0
