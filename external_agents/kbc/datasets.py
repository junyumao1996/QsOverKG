...
class Dataset_simple(object):
    def __init__(self, data, to_skip):
        ...
        self.n_entities = int(max(maxis[0], maxis[2]) + 1)
        self.n_predicates = int(maxis[1] + 1)
        self.n_predicates *= 2  # contain the receporicals 
    ...
    def get_train(self):
        """
        Concats the receporical training triples. 
        """
        ...
        copy[:, 2] = tmp
        copy[:, 1] += self.n_predicates // 2  # has been multiplied by two.
        return np.vstack((self.data, copy))

    def eval(
            self, model: KBCModel, n_queries: int = -1, missing_eval: str = 'both',
            at: Tuple[int] = (1, 3, 10)
    ):
        test = self.get_examples()
